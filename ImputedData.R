...
#NOTE: I use column nubers rather than names due to capitalization differences between CBP years
Data <- Initial_data[,c(1:24)]
Data <- Data[,-c(7:10,20)]

#Remove Initial_data for memory purposes
rm(Initial_data)
...
#If no EMPFLAG, then Min = Min, else Min is matrix multiplication of enterprises * enterprise size data
Data$Min2 <- ifelse(Data[,3]=="", 
                      Data$Min,
                      as.matrix(Data[, 6:17]) %*% as.matrix(EntCodeRang$Min))

#Min = maximum of Min and Min2
Data$Min3 <- with(Data, pmax(Data$Min, Data$Min2, na.rm = T))
...
Data$SecondEst <- (Data$Min3+Data$Max3)/2
...
#removing columns other than EMP, EST, and those generated by calculations above
Data <- Data[, c(1, 2, 5, 26, 33:42, 18)]
...
## Export Imputed and Establishments each cycle
write.csv(Imputed, "County/Imputed_Hist.csv", row.names = F)
write.csv(Establishments, "County/Establishments_Hist.csv", row.names = F)


#Testing for duplication error (this occurred in earlier versions, and I did not record why, so still check)
