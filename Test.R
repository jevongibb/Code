...
### Load Combo Matrix
Matrix <- read.csv("Matrices/AvgDist_Emp_2011to2015.csv", header = T, sep = ",", check.names = F, row.names = 1)
...
Boundary <- .45
Boundary <- quantile(edges$weight, Boundary)

new_edges <- subset(edges, weight<Boundary)
...
#Flip weight so that closer is larger
new_edges$weight <- abs(new_edges$weight-max(new_edges$weight))

edges <- new_edges
colnames(edges) <- c("Source", "Target", "Weight")
edges$Weight <- scale(edges$Weight)
edges$Weight <- edges$Weight - min(edges$Weight) + 1 # Gephi wants min value of 1
