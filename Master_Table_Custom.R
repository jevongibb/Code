...
#In case you want to generate custom CZ
Custom_Employees <- read.csv("County/Imputed_Concord.csv", header = T, sep = ",", stringsAsFactors = F, check.names = F)
Custom <- read.csv("CZ/Custom_Columbus.csv", header = T, sep = ",", stringsAsFactors = F, check.names = F)
Custom_Employees <- Custom_Employees[,c(1,2,13,12,11,10,9,8,7,6,5,4,3)]
Custom_Employees <- subset(Custom_Employees, County %in% Custom$County)
Custom_Employees$CZ <- 1001
Custom_Employees <- Custom_Employees[,c(14,2:13)]
Custom_Employees <- Custom_Employees %>% group_by(CZ, naics) %>% summarise_all((funs(sum(., na.rm = TRUE))))
...
#Addition for Custom CZ###############
CustomCZTotal <- Custom_Employees %>% group_by(CZ) %>% summarise(CZ_Total=sum(`2015`))
Custom_Employees <- Custom_Employees %>% left_join(CustomCZTotal, by="CZ")
Custom_Employees <- Custom_Employees %>% left_join(EmpNAICSTotal, by="naics")
Custom_Employees$RS_2015 <- (Custom_Employees$`2015` / Custom_Employees$CZ_Total) / (Custom_Employees$Industry_Total / EmpTotal)
######################################
